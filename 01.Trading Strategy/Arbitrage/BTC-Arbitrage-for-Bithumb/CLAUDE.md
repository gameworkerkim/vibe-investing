...
> 문서: [`readme.md`](./readme.md) · [`docs/DEVELOPMENT-PLAN.md`](./docs/DEVELOPMENT-PLAN.md)
...
- **환산율 기준(`FX_MODE`)이 시그널의 의미를 바꾼다**: `usdt`(기본)는 USDT 프리미엄이 상쇄된 **실행 가능 스프레드**(실측 ±0.1% 수준), `fx`는 USDT 프리미엄을 포함한 **헤드라인 김프**. 임계값은 기준에 맞춰 조정할 것
...
6. **순이익은 `|프리미엄|` 기준**: 차익거래는 방향과 무관하게 절댓값만큼 먹는 구조라 부호를 그대로 두면 역방향이 손실처럼 보인다
7. **KV 는 JSON**: `NaN` 을 저장할 수 없으므로 값 없음은 처음부터 `null`(`MaybeNumber`)로 표현한다
8. **KV 쓰기 예산**: 무료 플랜 1,000 write/일 · 5분 크론 = 288 스캔/일. 스캔당 쓰기는 스냅샷+히스토리 2회로 유지하고 알림 상태는 변경 시에만 쓴다
9. **관리 API 는 `Authorization: Bearer` 우선**. 쿼리스트링 토큰은 로그·히스토리에 남으므로 하위 호환용으로만 유지. 비교는 상수 시간, 오류·관리 응답은 `no-store`
10. **공개 시세는 폴백 필수**: 지역 차단·일시 장애 견딜 수 있게 이중 경로 + 8초 타임아웃. 한 코인이 빠져도 나머지 스캔은 계속한다
11. 한국어 UI 기본. 등락색은 한국식(적=상승 프리미엄, 청=하락 프리미엄)
...
docs/                개발 계획 등 문서 (assets/dashboard.png = 대시보드 스크린샷)
