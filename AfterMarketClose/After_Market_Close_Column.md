...
현대 실적 발표는 정보량이 많다. 10 ~ 30페이지의 earnings release, 관련 8-K, 재무제표, 경영진 코멘트, 때로는 Q&A 트랜스크립트까지 동시에 쏟아진다. 장중에 이 모두가 쏟아지면 초기 밀리초 단위 알고리즘 트레이딩이 부분 정보만으로 극단적 반응을 일으킬 수 있다. AMC 공시는 투자자·애널리스트가 최소 17.5시간(16:00~다음날 09:30)동안 정보를 읽고 모델을 업데이트할 수 있도록 해준다.
