...
본 서비스는 Quant 학습용으로 개발된 서비스이며, 헤지펀드의 퀀드 알고리즘을 학습하기 위한 교육 샌드박스이며, 투자 자문이 아닙니다.
기본 컨셉은 전통적인 Quant 알고리즘을 이해하기 위해 GS Quant와 유사한 퀀트 API와 별도의 데이터넷을 기준으로 서비스가 구성됩니다. 내가 파이썬에 능통하다면 파이썬으로 제공된 API를 통해 인사이트를 얻을 수 있습니다.

이는 전통적인 퀀트 알고리즘 개발입니다. 여기에 LLM으로 원하는 퀀트 프롬프트를 입력하여 파이썬 퀀트 코드를 만들 수 있습니다.
우리가 잊지 말아야할 것 한가지.
