...
from vi_quant.api.vi.data import ViDataApi, QueryType
from vi_quant.common import Currency
from vi_quant.markets.securities import Asset
...
def excess_returns(
    ...
    *,
    ...
    with DataContext(price_series.index[0], price_series.index[-1]):
        q = ViDataApi.build_market_data_query([marquee_id], QueryType.SPOT)
        df = ViDataApi.get_market_data(q)
    if df.empty:
