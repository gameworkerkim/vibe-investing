...
MAI-Cyber-1-Flash를 이해하려면 계보를 알아야 한다. 초판에서 이 부분이 가장 크게 틀렸다.
...
즉 MAI-Cyber-1-Flash는 플래그십 추론 모델의 축소 계열이며, 활성 파라미터가 5B에 불과한 **초경량 라우팅 워크호스**로 설계됐다. 이 점이 뒤에 나올 TCO 논리의 전부다.
