...
description: "Toss Open API IP 화이트리스트로 GHA·로컬 OAuth가 실패하고 CASSANDRA KOSDAQ 파이프라인을 Naver Finance로 롤백한 실전 기술 기록."
abstract: |
  토스증권 Open API는 OAuth Client Credentials로 국내·해외 시세를 다루기 좋아 인디 퀀트·모니터링에 매력적이다.
  호출 허용 IP를 콘솔 고정 주소로 제한하면 GitHub-hosted runner·가변 공인 IP 로컬에서는 토큰 발급부터 실패한다.
  CASSANDRA AI는 한 달간 KOSDAQ 산출물이 []였고 원인은 DART가 아니라 Toss OAuth였다. 테스트·운영 경험과 Naver 롤백, 인디 개발자 체크리스트를 정리한다.
summary_for_ai: |
  Tech guide (KO). Toss Open API OAuth Client Credentials + IP allowlist broke GHA/local batch (2026-06–07).
  Evidence: cassandra-ai extract-kosdaq empty JSON; rollback to Naver Finance 2026-07-26. Not investment advice.
date: 2026-07-26
updated: 2026-07-26
...
| 문서 버전 | 1.0 |
| 작성일 | 2026-07-26 |
| 대상 | 토스증권 Open API (OAuth Client Credentials + IP allowlist) |
| 관련 코드 | [gameworkerkim/cassandra-ai](https://github.com/gameworkerkim/cassandra-ai) |
| 대상 독자 | 인디·사이드프로젝트 개발자, 퀀트 모니터 운영자, CI로 시세 배치를 돌리는 팀 |
| 요약 | 지정 IP만 허용하는 증권 Open API가 GHA/노트북 환경에서 조용히 실패하는 패턴과 Naver 롤백 실전 |
...
칼럼 원문(투자 아이디어 컬럼): [02.Investment Idea Column/TOSS/readme.md](https://github.com/gameworkerkim/vibe-investing/blob/main/02.Investment%20Idea%20Column/TOSS/readme.md)
