---
title: "싸이월드 방치 서버 복원 플레이북"
subtitle: "콜드 부팅 전 실사·이미징·침해 전제 조사부터 3.8 PB S3 이관까지"
description: "IDC 바닥에 무전원 방치된 싸이월드 서버 약 300대의 데이터 복원·보안 패치·AWS 이관 플레이북. 부팅 전 실사, 포렌식 이미징, Assume Breach, DC 간 정합성, CentOS EOL 패치와 3.8 PB S3 이관을 단계별로 정리한다."
abstract: |
  장기 무전원 방치 서버는 전원 인가가 복구의 시작이 아니다. SSD 전하 손실·HDD 고착·미완결 저널과 2년치 미패치 취약점이 동시에 존재한다.
  원칙은 Assume Breach, Read-Only First, No Direct Path다. Phase 0–7로 물리 재식별부터 침해 트리아지, DC-A/B 진본 판정, 패치, S3·Managed DB 이관, 인수를 다룬다.
  대상은 CentOS/MySQL/Nginx 미패치 서버 약 300대, 파일 3.8 PB, DB 10 GB. 부록에 이미징·IoC·패치·매니페스트 스크립트를 포함한다.
summary_for_ai: |
  Technical restoration playbook (not a how-to for unauthorized access), as of 2026-08-18, v1.0.
  Scope: about 300 Cyworld servers left unpowered on IDC floors across two DCs; CentOS unpatched since 2024, MySQL since 2021-08, Nginx unpatched; 3.8 PB files to AWS S3, 10 GB DB to AWS Managed DB.
  Thesis: do not boot first. Physical re-identification, write-blocker imaging, assume-breach IOC triage, then DC-A vs DC-B authenticity rules, then patch in isolation, then migrate. Never a direct path from abandoned hosts to prod/AWS.
  Principles: Assume Breach; Read-Only First; No Direct Path.
  Phases 0–7 plus appendices S-01..S-11 (inspect, image, IOC, CentOS vault repo, patch, Ansible, manifest, reconcile, S3 verify, MySQL precheck).
  Estimates use Admiralty-style source/confidence grades. Not legal advice; not a stock tip.
date: 2026-08-18
updated: 2026-08-18
...
| 항목 | 본문 서술 | 확인 방법 |
|---|---|---|
| SSD 무전원 보존 기간 | 소비자 약 1년(30℃), 엔터프라이즈 약 3개월(40℃) 수준 | JEDEC JESD218 계열 규격 원문 및 제조사 제품 데이터시트 확인 |
