...
## 9. 파생 트랙: LAON Code Dojo — 보안 교육 플랫폼

> **상세 문서**: [LAON_CODE_DOJO.md](./LAON_CODE_DOJO.md)

LAON VaultGuard의 시크릿 탐지 엔진을 **보안 교육**으로 전환한 파생 프로젝트.

| 항목 | LAON VaultGuard (기존) | LAON Code Dojo (신규) |
|------|----------------------|---------------------|
| **목적** | 시크릿 탐지·차단 | 개발자 보안 교육 |
| **대상** | 보안 엔지니어, DevOps | **주니어·중급 개발자 (1~5년차)** |
| **형식** | CLI + VS Code 확장 + 웹 대시보드 | **퀴즈 플랫폼** (코드 조각 → 취약점 찾기 → 해설) |
| **LLM 활용** | 코드 스캔 (탐지) | **문제 생성 + 채점 + 실시간 해설** |
| **경쟁 환경** | GitHub·GitGuardian·TruffleHog와 직접 경쟁 | **교육 시장** — 경쟁사 대비 무료·한국어·오픈소스 차별화 |
| **수익화 경로** | SaaS 전환 어려움 (시장 포화) | Free + Pro($9/월) + Team($49/월) |
| **인프라** | 동일 (Vercel + Neon + Cloudflare CDN) | 동일 — 모노레포 `packages/dojo/` 추가 |

**핵심 전환 논리**:
1. 시크릿 스캐너 시장은 GitHub·GitGuardian 등 자본력 있는 플레이어가 지배 중 → 신규 진입자 설 자리 감소
2. 보안 교육은 **덜 붐비는 시장**. 특히 개인 개발자 대상 무료·한국어 서비스는 공백 상태
3. LAON VaultGuard가 이미 보유한 멀티 LLM 하네스 + 60+ 탐지 패턴 + SARIF Export를 그대로 교육 플랫폼으로 재활용 가능
4. "도구를 팔기 어려우면, 도구 사용법을 가르쳐라" — 교육받은 개발자가 결국 스캐너 사용자로 전환

**vvibequant.cc Lab 연동**: `lab.vibequant.cc/dojo` → MY-IP·VaultGuard 데모와 함께 "오늘의 보안 퀴즈" 제공
