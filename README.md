...
## NEW · DART Monitor — Vercel 앱 (Lab 탭 아님)

> 코스닥 DART 공시 모니터. Lab HTML과 앱의 GitHub 경로는 다르다.

**앱**은 [dart-monitor-pi.vercel.app](https://dart-monitor-pi.vercel.app/) 에 있다. Vercel Git 소스는 [`github.com/gameworkerkim/cassandra-ai`](https://github.com/gameworkerkim/cassandra-ai) **루트 `./`** (package.json 이름 `dart-monitor`). 모노레포 폴더 `CASSANDRA AI/`(공백 있음)는 사본일 뿐 Vercel Root가 아니다.

**Lab 페이지** `https://vibequant.cc/lab/` 는 Cloudflare Pages 프로젝트 `vibequant-web`이 [`VibeQuant/pages/lab/`](https://github.com/gameworkerkim/vibe-investing/tree/main/VibeQuant/pages/lab) 를 올린 결과다. 기본 탭은 TokenForge. `TokenForge/` 나 `CASSANDRA AI/` 를 Pages 루트로 지정하면 `/lab/` 이 안 바뀐다. 배포: `cd VibeQuant/pages && npx wrangler pages deploy . --project-name=vibequant-web`.
...
**화면 매뉴얼 (컨셉 → 계획 수립 → 토큰 최적화, 스크린샷):** [`TokenForge/docs/LAB_MANUAL.md`](https://github.com/gameworkerkim/vibe-investing/blob/main/TokenForge/docs/LAB_MANUAL.md)
