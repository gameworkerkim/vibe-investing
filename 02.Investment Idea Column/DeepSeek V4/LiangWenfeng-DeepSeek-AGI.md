...
그의 IR의 흐름을 보면 DeepSeek의 그림이 하나로 맞춰진다. **개발 철학**은 "제품은 부산물이고 지능이 본류이며, 다음 세대 모델은 먼저 우리 자신을 위해 만든다"로 요약된다. **투자 철학**은 "파이를 독식하지 말고 실현 확률을 높여라, 오픈소스와 저가가 곧 그 확률을 높이는 전략"으로 요약된다. 두 축의 접점에 '절제'가 있다. 절제는 개발에서는 곁가지를 쳐내는 집중이고, 투자에서는 방향을 지키는 규율이다.
