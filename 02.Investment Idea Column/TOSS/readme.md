...
한 달 뒤 대시보드의 코스닥 이상징후 리포트는 `totalStocks: 0`, 하위 JSON은 전부 `[]`였다. DART 키도 살아 있었고, GitHub Actions도 초록불이었다. 원인은 공시가 아니라 **시세 OAuth가 지정 IP 밖에서는 발급되지 않는 보안 정책**이었다. API 발급 창을 보니 화이트 리스트 추가 기능이 들어갔다.

이 글은 비난이 아니라 **주의점**이다. 금융 Open API의 IP 바인딩은 합리적 보안이지만, 인디·사이드프로젝트·공개 CI와는 상성이 나쁘다. 테스트와 운영에서 실제로 겪은 패턴 중 하나이다. API가 죽거나 정책이 변경되거나 Limit가 생기는 일이 발생한다.

토스증권 측에서도 API request가 과도하게 나와서 시스템을 불안정하게 만들었을지도 모른다. 그래서 추가를 했거나 관계 당국에서 보안 강화의 목적으로 강제했을 가능성이 있다. 일단 서버에 API request가 디도스 급으로 왔을 것이라는 것이 한 표를 걸 수 있다. 바이브코딩은 생각보다 연결된 서버의 한계를 모르고 api 요청을 했을 것이고, 초단타 매매를 하고자 하는 개발자 수요에 부응하기 위해 웹소겟을 지원하지 않는 TOSS Open API 서버를 디도스처럼 초단시간에 불렀을지도 모른다.
...
## 4. 내가 내린 결정 - Naver로 롤백

2026-07-26 기준 CASSANDRA 배치·시세 경로는 다시 **Naver Finance 모바일 API**를 쓴다. 귀찮지만 다른 대안을 찾아서 안정적으로 데이터를 폴링할 방법을 찾을 예정이다. 
